...
# data['Value'] = pd.to_numeric(data['Value'], errors='coerce').ffill()
...
# # Define BiGRU Model with Entity Embeddings
...
#         self.gru = nn.GRU(input_size + embedding_dim, hidden_layer_size, num_layers=1, bidirectional=True)
...
#         gru_out, _ = self.gru(input_combined)
...
# model = BiGRU(input_size, hidden_layer_size, output_size, dropout_prob, num_entities, embedding_dim)
...
# scaler = RobustScaler()  # Use RobustScaler instead of MinMaxScaler
# data['Value'] = scaler.fit_transform(data[['Value']])
